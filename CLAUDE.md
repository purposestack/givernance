...
│   ├── 30-advanced-filters.md      — Advanced Constituent Filters (Epic #418, PR #421; reconciled by the advanced-filters audit): FE/BE catalog kept in lockstep (only backend-executable fields shown), query DSL + pattern detection (LYBUNT, SYBUNT, RECURRING, LAPSED, MAJOR_DONOR), user-friendly nullable operator (isNull/isNotNull = "is empty"/"has a value" on nullable columns only), EUR→cents value scaling, aggregate-existence + soft-delete + OR-across-aggregate correctness, preset templates, real-time preview, persisted per-campaign segmentation
