...
- **Rebasing onto `main`** when both your branch and main added migrations in parallel: re-check that `idx` is contiguous, `tag`s match filenames, and `when` is strictly increasing. **Never change the `when` of an entry that already shipped on `main`** — keep main's entries byte-for-byte and move *your* branch entries after main's max `when`. `when` values are a durable contract with every long-lived DB: drizzle records them as `created_at` and applies exactly the entries above `max(created_at)` (hashes are stored but never compared), so re-spacing makes migrated DBs skip never-ran migrations and re-apply existing ones (`already exists` abort). The 2026-07-21 multi-currency rebase hit both; the repair procedure is [`docs/runbooks/drizzle-journal-rebase-reconciliation.md`](docs/runbooks/drizzle-journal-rebase-reconciliation.md). Re-spacing *your own branch's* unmerged entries is the correct resolution — but every dev DB that migrated the branch pre-rebase then needs that runbook.
...
**Automated guard**: [`packages/api/src/tests/integration/migrations-journal-parity.test.ts`](packages/api/src/tests/integration/migrations-journal-parity.test.ts) fails CI on orphan files, phantom journal entries, gappy `idx`, non-monotonic `when` — and on any divergence from the committed baseline mirror [`packages/api/migrations/meta/_journal.baseline.json`](packages/api/migrations/meta/_journal.baseline.json) other than pure appends. After adding a migration, refresh the baseline in the same PR: `pnpm --filter @givernance/api run db:journal:baseline`. If the baseline test reports **changed `when` values on existing entries**, that's a re-spacing — reconcile affected long-lived DBs first ([runbook](docs/runbooks/drizzle-journal-rebase-reconciliation.md)), then refresh. If any of these tests go red after an edit, the journal/baseline is the thing to fix — never the test.
