...
| Feature flags | **Every net-new user-facing feature ships behind a flag, default-off** — see CLAUDE.md "Feature-flag-first rule" and `docs/18-feature-flags.md`. Five-step pattern: (1) register key in `FEATURE_FLAG_KEYS` + `FEATURE_FLAG_REGISTRY` at [`packages/shared/src/constants/feature-flags.ts`](../../packages/shared/src/constants/feature-flags.ts) — **dotted-domain naming `<domain>.<feature>`, NOT `ff.`-prefixed** (the `ff.` form lower in this doc is stale); (2) seed migration `INSERT … ON CONFLICT (key) DO NOTHING`; (3) `requireFlag(FEATURE_FLAG_KEYS.YOUR_KEY)` as the FIRST preHandler on every new route, before role guards; (4) `isFlagEnabled(...)` at worker job pickup for defence-in-depth; (5) SSR-fetch `/v1/feature-flags` and hide every dependent UI surface. Off-state QA: surface completely absent (route 404s, button hidden, column hidden). Skip the flag only for bug fixes, pure infra, or refactors with no behaviour change. |
