 ...
 * pure scoring function `computeMobilisationScore` consumes. Lives on the
 * super-admin surface — never tenant-facing — and runs on the owner pool
 * because the platform aggregate is cross-tenant by design.
 *
 * SHARED (issue #443): the `db` handle is injected so the API (manual
...
} from "../../mobilisation/score.js";
import type * as schema from "../../schema/index.js";

/** Injected Drizzle handle (owner pool — the aggregate is cross-tenant). */
...
export interface ComputePerTenantOptions {
  /** Optionally narrow to one tenant (still cross-tenant pool for consistency). */
  tenantId?: string;
}
 ...
 * aggregation across all tenants; callers (super-admin route, monthly
 * cron) are authorised at their own layer and pass the owner-pool `db`.
 */
...
export async function computeMobilisationPlatformAggregate(
  ...
  const tenants = await computeMobilisationPerTenant(db, period);
