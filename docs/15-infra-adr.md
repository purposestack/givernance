...
2. **Tenant claim sanity check** — every authenticated request must satisfy ONE of:
   ...
   Anything else (a JWT with `sub` but no `org_id` and no `super_admin` role, or a JWT whose `(sub, org_id)` doesn't resolve to an active row) is rejected at the auth plugin layer with 401 + `authDenial: { reason: "no_active_membership" }`. This closes the symptom reported in PR #185 round-3: a deleted user with a still-valid JWT could reach `/dashboard` because no route checked that the JWT's `(sub, org_id)` actually corresponded to a member of any tenant.
