...
- **Removal guards (issue #614)** — `DELETE /v1/users/:id` refuses, inside the soft-delete transaction, to remove the caller's own row (422 `errorCode: cannot_remove_self`) and to remove the last ACTIVE `org_admin` of the tenant (422 `errorCode: cannot_remove_last_admin`) — the same shape as the `PATCH /v1/users/:id` role guards (`cannot_self_demote` / `cannot_demote_last_admin`). Both last-admin counts exclude soft-deleted rows (`deleted_at IS NULL`). The members table hides "Remove" on the caller's own row; the API gate is the durable enforcement. A zero-admin tenant can only be recovered by a super-admin.
