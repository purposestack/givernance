...
- Silent refresh is now load-bearing. A refresh failure ≠ "user is gone" — it could mean transient Keycloak unavailability. The AuthProvider retries with a budget (5 attempts at 30s) before clearing local state — for a browser-side network error and for the refresh route's own `503` (Keycloak unreachable, cookies kept) alike.
- Silent refresh must never touch an impersonation session (issue #613). During impersonation `givernance_jwt` holds the non-renewable impersonation token while the `/api/auth`-scoped refresh cookie still belongs to the operator; rotating would silently swap a read-only / delegated session for a plain super-admin one and lose the `act` double attribution. `POST /api/auth/refresh` therefore answers a `200` no-op (`{ ok: true, skipped: "impersonation" }`, no cookie writes, no Keycloak call) whenever the session cookie is an impersonation token of either flavour (app-layer `iss`, or realm-signed with an `act` claim). The operator's session comes back through the explicit full-page `GET /api/auth/restore-session` navigation once the impersonation token is dead — see `docs/19-impersonation.md` §3.
