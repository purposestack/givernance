...
6. **Resume the relay deployment AND restart the worker container.** The relay restores outbox event flow; the worker restart re-runs `scheduleRepeatableJobs()` (`packages/worker/src/worker.ts:59-69`) so BullMQ's repeatable schedule is re-registered into the fresh Redis. Without the worker restart, the daily 03:15 UTC `tenant.provisional-admin-expire` cron silently stops firing until the next deploy (could be days or weeks). The worker's ioredis connection auto-reconnects when Redis comes back, but BullMQ's `Worker` only reattaches to existing queues — it does not re-emit the repeatable-job registration, which is what populates `bull:tenant_lifecycle:repeat`. The wipe in step 4 erased that key; only re-running the registration code restores it.
   - Restart: `ssh givernance-staging 'docker restart $(docker ps --filter name=givernance-worker --format "{{.Names}}")'`.
   - Verify: `ssh givernance-staging 'docker exec givernance-redis redis-cli ZRANGE bull:tenant_lifecycle:repeat 0 -1 WITHSCORES'` — must return exactly one entry (the `tenant-provisional-admin-expire-daily` schedule and its next firing timestamp). An empty result means the worker restart didn't take and the cron is dead until further action.
7. **Confirm new outbox events flow through** end-to-end (relay → events queue → worker handler).

> **Why this gap exists.** `scheduleRepeatableJobs()` is called exactly once per worker process — at startup (`packages/worker/src/worker.ts:466-468`). A Redis wipe drops the `bull:tenant_lifecycle:repeat` ZSET; ioredis reconnects transparently and the `Worker` instances reattach to their queues, but neither path re-invokes the registration. The same gap will apply to any future repeatable job we add. A more durable fix — calling `scheduleRepeatableJobs()` from the worker's `redis.connect` handler so reconnection also re-registers — is a separate architecture decision and is **not** covered by this ADR. An `assert-cron-registered` health check that fails the worker if the schedule isn't present after startup is a second, complementary defensive option. Both are explicitly out of scope here; the doc fix above is the immediate operational need.
