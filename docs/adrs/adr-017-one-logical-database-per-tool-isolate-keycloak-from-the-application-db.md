...
- **Per-tool CVE visibility.** The corollary of "each tool gets its own DB and its own blast radius" is that each tool also needs its own CVE feed. `.github/dependabot.yml` covers the four `Dockerfile`s in the repo (root, `infra/keycloak`, `packages/api`, `packages/worker`) via the `docker` ecosystem, **plus** the seven runtime-path images pinned only inside `docker-compose.yml` (`postgres:16-alpine`, `redis:7-alpine`, `quay.io/keycloak/keycloak:26.6.1`, `minio/minio:…`, `minio/mc:…`, `axllent/mailpit:…`, `caddy:2-alpine`) via the `docker-compose` ecosystem (GA February 2025 — see the [GitHub changelog](https://github.blog/changelog/2025-02-25-dependabot-version-updates-now-support-docker-compose-in-general-availability/)). Findings surface as Dependabot PRs against the relevant file, on the same alerts feed as npm and `docker` Dockerfile CVEs. The Kamal staging accessories in `config/deploy-staging.yml` re-pin some of the same upstream images on intentional floating tags (`postgres:16`, `redis:7`, `minio/minio:latest`); base-image CVEs there are picked up at the next deploy from the upstream registry. The custom-built `ghcr.io/purposestack/givernance-keycloak` image referenced by Kamal already inherits scanning via its `infra/keycloak/Dockerfile` `FROM` line. Renovate, a parallel `Dockerfile.scan` per service, and out-of-band Trivy/Grype scans were considered and rejected: a single Dependabot configuration covers every image in the runtime path and keeps the alerts feed consolidated under one tool.
