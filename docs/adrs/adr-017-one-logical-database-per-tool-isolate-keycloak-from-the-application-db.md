...
- **Staging (Kamal on Scaleway VM).** The live staging Postgres cluster pre-dated this ADR — it was first bootstrapped with Keycloak co-located in `givernance_staging`. The one-shot blue-green cutover runbook at [`docs/runbooks/migrate-staging-keycloak-db.md`](../runbooks/migrate-staging-keycloak-db.md) (issue [#283](https://github.com/purposestack/givernance/issues/283)) brings the running cluster into compliance: temp-ghost Keycloak via `docker run` on the kamal network, dump → restore of KC tables into a dedicated `givernance_keycloak` DB owned by a least-privilege `keycloak` role, ~30s `auth.staging.givernance.org` downtime for the `kamal accessory reboot keycloak` that swaps `KC_DB_URL`, then drop the KC tables from `givernance_staging`. The runbook is also the operator's journal — it lands in the cutover PR with the actual timings, row counts, and any deviations filled in. The wiring landed earlier (postgres accessory `files:` mount of `infra/postgres/init/01-init-keycloak-db.sh` + `KEYCLOAK_DB_*` env vars + new `KEYCLOAK_DB_PASSWORD` secret) so a future fresh-init reproduces the dedicated DB without operator intervention.
...
- **Per-tool CVE visibility.** The corollary of "each tool gets its own DB and its own blast radius" is that each tool also needs its own CVE feed. `.github/dependabot.yml` covers the four `Dockerfile`s in the repo (root, `infra/keycloak`, `packages/api`, `packages/worker`) via the `docker` ecosystem, **plus** the seven runtime-path images pinned only inside `docker-compose.yml` (`postgres:17-alpine`, `redis:8-alpine`, `quay.io/keycloak/keycloak:26.6.1`, `chrislusf/seaweedfs:4.31`, `amazon/aws-cli:…`, `axllent/mailpit:…`, `caddy:2-alpine` — the object store moved from MinIO to SeaweedFS per ADR-034) via the `docker-compose` ecosystem (GA February 2025 — see the [GitHub changelog](https://github.blog/changelog/2025-02-25-dependabot-version-updates-now-support-docker-compose-in-general-availability/)). Findings surface as Dependabot PRs against the relevant file, on the same alerts feed as npm and `docker` Dockerfile CVEs. The Kamal staging accessories in `config/deploy-staging.yml` re-pin some of the same upstream images on intentional floating tags (`postgres:17`, `redis:8`, `chrislusf/seaweedfs:4.31`); base-image CVEs there are picked up at the next deploy from the upstream registry. The custom-built `ghcr.io/purposestack/givernance-keycloak` image referenced by Kamal already inherits scanning via its `infra/keycloak/Dockerfile` `FROM` line. A parallel `Dockerfile.scan` per service and out-of-band Trivy/Grype scans were considered and rejected: a single Dependabot configuration covers every image in the runtime path and keeps the alerts feed consolidated under one tool. Renovate was rejected at the same time, but the manifest-driven compliance ceiling later added in **[ADR-026](./adr-026-postgres-17-cutover-and-scaleway-anchored-versioning.md)** delivers Renovate's `allowedVersions` semantics on top of Dependabot via a CI gate, so we get major-version policy enforcement without migrating tools.
