...
7. **Data draws directionally.** Time-series charts sweep left→right over `--duration-sweep` (600 ms): ghost points lead, value fill follows (clip-path/`scaleX` sweep or the chart library's native draw-on, whichever composites). Progress meters fill left→right from 0 to value. Timeline bars sweep. KPI numbers count up over the same window, landing together with their trend badge. Nothing "pops" to its final value while a sibling animates.
...
18. **Compositor properties only.** All choreography animates `opacity` and `transform` (or `clip-path` for draw-ons). No `width`/`height`/`top`/`left`/`margin` animation — meters fill via `scaleX` with `transform-origin: left`.
19. **Purposeful, not decorative** (restating `docs/11` §2.6 as the tie-breaker): if a proposed animation doesn't communicate *loading order*, *data freshness*, or *cause-and-effect*, it doesn't ship. `--ease-spring` stays out of data surfaces entirely.
...
- `docs/design/shared/tokens.css` and `packages/web/src/app/globals.css` gain `--stagger-step: 50ms` and `--duration-sweep: 600ms`; `docs/11-design-identity.md` §2.6 links here.
