...
19. **Invisible must not be interactive-hostile.** Entrance delays keep elements clickable but unseen; bound that window: table rows cap their cascade at 6 steps on a local 25 ms stagger (worst row visible < 500 ms), and any focused element snaps visible instantly via **timing collapse** (`:focus-visible`/`:focus-within` → `animation-duration: 0.01ms; animation-delay: 0s`). Never implement the snap as `animation: none`: toggling `animation-name` restarts the entrance the moment the rule stops matching — e.g. opening a `<select>` moves focus into its *portaled* dropdown, the form loses `:focus-within`, and the whole form replays its entrance. Changing only timing values recomputes the animation timeline without ever restarting it.
