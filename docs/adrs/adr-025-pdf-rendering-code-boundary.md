...
- **CI parity test** (shipped — issue #289): `packages/api/src/modules/campaigns/postal-pdf.parity.test.ts` renders five frozen fixtures (named + logo + description / door-drop fallback copy / cross-border address with lowercase country code / partial address + null locale + blank mission / 2-page overflow forcing auto-pagination) through BOTH files and asserts SHA-256 equality after masking the only legitimately-varying bytes: the `(D:YYYYMMDDHHmmssZ)` date literal — pdfkit writes Info-dict values as *indirect objects*, so the mask targets the literal, not a `/CreationDate (…)` key pair — and the `/ID [<…> <…>]` trailer, which pdfkit 0.19.1 emits **unconditionally** from a millisecond-resolution hash (the `/ID` mask is load-bearing, not future-proofing). Object numbers, xref offsets, and stream contents are deliberately NOT stripped — a difference there is a real drift. Failing the test blocks the PR. Self-tests guard the guard: the masks must demonstrably fire on real output (a PDFKit serialisation change fails deterministically instead of degrading into a same-second flake), a metadata-visible drift AND a compressed-stream-only drift must each fail (so an over-broad canonicaliser can't blind the guard), and same-input renders through each path must hash-match (a future salt points at the canonicaliser, not at a phantom drift). On mismatch the assertion prints the first differing byte offset + context from both sides. The worker renderer is loaded via a runtime-computed dynamic import (test-only, api → worker) so the package boundary and the "worker never imports api" direction stay intact; a static cross-package import would fail `tsc --noEmit` with TS6059 by design. **Scope**: the parity harness covers the standard-rail letter only — the compressed-appeal pair (`renderCompressedAppealForQrBill` ↔ `swiss-qr-bill-preview.ts`, ADR-027) remains socially enforced until the harness is extended (deferred while PR #361 rebuilds that zone).
