...
7. **One interaction curve, one duration — and form-control focus parity.** All hover/press feedback uses `transition-colors duration-normal ease-out` (150ms, `cubic-bezier(0.16, 1, 0.3, 1)`); focus is the instant primary ring (`shadow-ring` + `border-primary`). The global `prefers-reduced-motion` block neutralises it. **Focus parity rule**: text fields use `focus-visible:` (browsers grant it on click for typing surfaces), but *field-like `<button>` triggers* — `SelectTrigger`, combobox/date-picker popover triggers — must use plain `focus:` variants, because browsers only grant `:focus-visible` to buttons on keyboard focus: with `focus-visible:` a pointer click shows no ring while the neighbouring input does, which reads as an inconsistency. Toggle controls (checkbox, radio, switch) keep `focus-visible:` — a persistent ring after a click-toggle is noise, and that matches the native idiom.
