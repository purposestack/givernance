...
| **Worker job replay** | **Not resume-safe yet**: a re-run of a `processing` job restarts from row 0 (counters are incremented blindly, rows already committed would be processed again). The `bulk_import` queue therefore runs with `attempts: 1` (issue #612 — `packages/shared/src/jobs/queue-options.ts`); a job already in a terminal state (`completed` / `partial` / `failed`) short-circuits. On a crash the operator re-uploads. Making the processor idempotent on `(job_id, row_number)` — skip rows already in `bulk_import_results`, recompute `processed_rows` from that table — is the follow-up that unlocks retries. | Worker `processBulkImport` |
