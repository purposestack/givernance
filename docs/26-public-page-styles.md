...
### 5.1 Donor-page guarantees (issue #615)

- **Fallback guarantee.** `/p/[id]` always passes the hardcoded (pre-Epic-362) layout — a complete page with its own live donation form — to `<ArchetypeRenderer>` as `fallback`. It is what the server renders. If the archetype's slot bundle fails to load (or the style key is invalid), the fallback is shown immediately and the donor can still give. While the bundle is loading, the fallback stays in the DOM but is held invisible by `.archetype-fallback-pending` (`_shell.css`) and revealed by a pure-CSS 3 s delay — the fast path never flashes one design and swaps to another, and the slow / no-hydration path still ends on a working page.
- **One form instance per visit.** The fallback and the archetype each mount their own `<PublicDonationForm>`, so the renderer never swaps once the fallback is in use: it settles on the fallback (a) on a Stripe return (`?payment_intent_client_secret=` — the first instance resolves the PaymentIntent and cleans the URL; a second instance would show an empty form to a donor who just paid), and (b) when the donor has already focused a field of a revealed fallback.
- **Landmark.** The page owns the single `<main id="main-content">` on both branches (target of the root skip link). Neither the renderer nor the hardcoded layout renders a `<main>` — the renderer is also mounted in the campaign-editor preview, under the app shell's `<main>`.
- **Slot i18n.** Slots are client components under the root `NextIntlClientProvider`. `useProgressModel` formats amounts with the page locale (`useLocale()`) and the campaign's `defaultCurrency`; slots never pass a literal locale to `Intl`. Every donor-visible word comes from `publicDonationPage.*` (`progress.labels.*`, per-archetype sentences under `progress.meta.<archetype-key>` with ICU `plural` for the donor count, `footer.poweredBy`). An archetype's typographic voice (UPPERCASE, lowercase) is `text-transform` in its `tokens.css`, never the casing of the string.
- **Metadata.** `generateMetadata` on `/p/[id]` emits title (`<campaign> — <organisation>`), description, Open Graph / Twitter tags and a canonical URL of `/p/<id>` **without** query params. The variant params `?style=` (archetype preview) and `?qr=` (postal attribution token) are `noindex, nofollow`. It shares one API call with the page through React `cache`. An unpublished / unknown campaign renders the donor-facing `not-found.tsx` of the segment (no operator search box, no dashboard link); render failures hit the segment's `error.tsx`.
