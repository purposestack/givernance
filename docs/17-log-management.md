...
**Direct-enqueue exception — signup resend (issue #575)**: `POST /v1/public/signup/resend` bypasses the outbox at request time (it enqueues BullMQ directly so the 204 stays constant-time, F3). The trace context therefore rides in the **job payload** (`SignupResendJobPayload.metadata`) instead of an outbox row; the worker re-validates the traceparent (the Redis payload is a second trust boundary) and stamps it onto the `tenant.signup_verification_resent` outbox insert it makes. Cron-originated passes with no inbound context (e.g. the tenant-lifecycle nightly expire) root one synthesised trace per pass, derived deterministically from the repeatable job id.
