...
| CSV export | constituents module | One column per `exportable` def, option ids → labels, ordered by `sort_order`; header = label, falling back to the import template's stable `cf_<key>` alias when the label would collide with a core import header or another definition — so an export/import round-trip lands on the **same** custom field, never a core column; honors the active `?filters=` DSL; **never plan-gated** |
| Bulk import | constituents `bulk-import/` + [`packages/worker`](../packages/worker/src/) | Template 1.1: one column per non-archived constituent def, header = label with stable `cf_<key>` alias; a file carrying **both** the label header and the `cf_<key>` alias of the same definition (or a repeated custom header) is rejected per-file, naming the keys — never "last non-empty cell wins"; worker validates via `buildCustomValidator`, resolves picklist labels case-insensitively to ids, never auto-creates options |
...
> **Epic deviation (Epic #539 § 4).** The epic specified `catalog_version`-keyed caching: a per-org monotonic counter baked into the cache key, so a bumped version makes stale entries unreachable. What shipped is **fixed per-domain keys + explicit `UNLINK` on every mutation + the 60 s TTL backstop**. Behaviourally equivalent for consumers (worst-case staleness = one TTL window, zero when invalidation fires, which every mutation path does — see the cache-invalidation integration test in `constituents-custom-values.test.ts`), with one less counter to persist and no orphaned versioned keys to expire. This is a deliberate deviation, not drift; also recorded in ADR-036 § Consequences.

**Sync vs. async.** Everything is synchronous request/response except three things: the **option-merge backfill** (dry-run is a sync count; the real merge returns 202 and rewrites values chunk-by-chunk in a BullMQ job — each row is rewritten under a re-checked containment predicate and its `custom_field_merge_undo` row is written only when that guarded rewrite actually touched the row), the **merge undo** (202; the worker **consumes first**: each chunk transaction deletes up to 500 undo rows with `RETURNING`, then restores `previous_value` only for the rows it just consumed), and the **undo-purge cron** (daily, deletes undo rows past `expires_at`). **Erasure serialisation**: every backfill / restore chunk transaction and the GDPR-erasure transaction take the same per-org `custom_fields:values` advisory lock (transaction-scoped), so the pipelines cannot interleave with an erasure at statement level — see § 6. The merge/undo handoffs are **transactional**: the API writes the outbox row inside the same transaction as the option-state change, and the relay enqueues with a deterministic per-merge jobId — a crash or Redis outage between "option marked merged" and "backfill enqueued" cannot happen. **Merge markers are API-written only**: the backfill verifies at pickup that the source option still carries *its* job's `mergeId` marker and **skips** (never re-marks) when an undo — or a subsequent merge — intervened between outbox emit and job pickup; a delayed merge job can therefore never brick an undone option or silently reverse a completed undo. The backfill also re-checks `isFlagEnabled` at pickup (defence-in-depth); the purge cron runs unconditionally — retention enforcement, like erasure, is never flag-gated.
...
**Filter execution.** Picklist/boolean equality compiles to `custom @> '{"key":"opt_x"}'` and is served by the GIN `jsonb_path_ops` indexes; number/date ranges run as unindexed cast expressions (`(custom->>'key')::numeric` / `::date`) — acceptable at NPO scale. **Every cast lane is type-guarded** (`jsonb_typeof` / strict date-shape `CASE`): a stored value that no longer matches its definition's type — the documented archive-and-recreate type-change path leaves old-typed values in blobs — evaluates as SQL NULL ("empty never matches"; sorts with the empties) instead of aborting the whole query with a cast error. The `/filter/fields` catalog serves picklist options in **`sortOrder` order** (not stored-array order), so an operator's option reorder reaches the FilterBuilder value dropdown. **Per-field expression indexes are the documented escape hatch** if a tenant's range filters ever breach the p95 budget: a deliberate, per-key migration decision — never automatic (open question: the exact p95 threshold that triggers one, and its ownership).
...
| `PATCH /v1/custom-fields/:id/options/:optId` | `requireFlag(domain) → requireAuth → requireOrgAdmin` | Rename-in-place / deactivate / reorder — never deletes. Reorder contract: the server persists the sent `sortOrder` **verbatim** (no renumbering); the client splices-and-renumbers the whole list, one PATCH per changed option. Two admins reordering the same picklist concurrently (or a mid-sequence PATCH failure) can transiently persist duplicate `sortOrder`s — accepted: display-order-only (values reference stable `opt_*` ids), rendered deterministically via stable sort, and fully repaired by the next successful move |
...
| `GET /v1/custom-fields/usage` | `requireFlag → requireAuth → requireOrgAdmin` | Fill-rate + per-option counts; counts < 5 rendered `'< 5'` (k-anonymity); multi-picklist option counts fence non-array residue (`jsonb_typeof = 'array'`), so archive-and-recreate leftovers count as filled but never abort the report |
| `POST /v1/constituents` · `PUT /v1/constituents/:id` | existing guards (`requireWrite`) | Optional `custom` merge-patch; validated only when `constituents.custom_fields` is on. Flag off: a payload **carrying values** is rejected 422; an empty `custom: {}` passes — uniform on create AND update, and identical across the three domains. Unknown key / inactive option → 422 error list |
| Donation / campaign create + update | existing guards | Same contract under `donations.custom_fields` / `campaigns.custom_fields` |
| `GET` detail/list of the three domains | existing guards | `custom` (and `donorCustom` projection on donations) added via definition-driven serializers when the flag is on; absent when off. **Detail** responses serialize active + archived definitions (values retained, read-only); **list** responses and the projection stay active-only. Archive-and-recreate dedupe: when an archived key was recreated, the **active definition wins** — the value serializes (API) and renders (web detail pages) exactly once, never a second time through the archived twin's stale type/options |
...
| Quota-override writes | operator runbook only (Phases 1–2) | No tenant-facing endpoint; rows written via owner-role SQL per [`docs/runbooks/customization-quota-override.md`](runbooks/customization-quota-override.md) with mandatory `reason` + a mandatory manual `custom_field.quota_override_set` audit row in the same transaction |
| Worker: `custom-field-option-merge-backfill` | `isFlagEnabled` re-check at pickup + `mergeId`-marker verification (skips — never re-marks — when an undo or later merge intervened) | `withWorkerContext(orgId)` + explicit `eq(orgId)`; counts-only audit rows |
...
**Audit actions**: `custom_field.created|updated|archived|restored|reordered`, `custom_field.option_added|renamed|deactivated|merged`, `custom_field.option_merged|option_merge_undone` (route-side, ids only), `custom_field.option_merge_backfilled|option_merge_undo_backfilled` (worker-side, counts + ids only — see § 6), `custom_field.quota_override_set|quota_override_revoked` (runbook-written in Phases 1–2, metadata only — see § 5), `constituents.exported` (CSV egress trail).
...
Archived definitions never count against a quota. Raises beyond a plan — but never beyond the platform ceiling — go through `customization_quota_overrides`: super-admin only, mandatory non-blank `reason`, optional expiry. A sales conversation with a paper trail, not a toggle. **Write path in Phases 1–2**: there is no super-admin endpoint yet — overrides are inserted by an operator via owner-role SQL following [`docs/runbooks/customization-quota-override.md`](runbooks/customization-quota-override.md), which **mandates a manual `audit_logs` INSERT (`custom_field.quota_override_set`, metadata only) in the same transaction, before the override row** — the runbook is the endpoint's stand-in and must leave the same audit trail. The tenant-side quota check reads the overrides (SELECT-only grant for `givernance_app`) and ignores rows past `expires_at`. The governed endpoint (reason-mandatory body, automatic audit row, RBAC/flag tests) is a fast-follow (§ 7).
...
- **Erasure × option-merge undo**: the erasure transaction **purges the subject's `custom_field_merge_undo` rows** — constituent-domain rows entirely (the whole blob was wiped), donation-domain rows whose definition is `sensitive=true` (matching exactly the Art. 9 strip; non-sensitive donation undo rows stay restorable with the retained financial row). A post-erasure merge undo can therefore never resurrect pre-erasure values. Defence in depth: the undo backfill's constituent UPDATE also carries `deleted_at IS NULL`, so a soft-deleted/erased row no-ops even if an undo row survived.
- **Erasure × concurrent worker pipelines** (statement-level races, not just orderings): the erasure transaction, every merge-backfill chunk, and every undo-restore chunk take the same **per-org transaction-scoped advisory lock** (`custom_fields:values:{orgId}`) as their first statement, so the three pipelines fully serialise per org — a backfill that scanned rows before an erasure committed can no longer re-add the merged key onto the wiped blob (or a stripped Art. 9 donation key, where no `deleted_at` wall exists) nor insert undo rows *after* the purge that would retain pre-erasure values for 30 days. Two further walls behind the lock: the backfill's per-row UPDATE re-checks the containment predicate (+ `deleted_at IS NULL` for constituents) and skips the undo-row INSERT when the re-check fails, and the undo restore is **consume-first** (`DELETE … RETURNING`, then restore only what it deleted), so a purge racing a restore serialises on the undo-row lock and a purged row is never replayed. Pinned by deterministic held-lock race tests on the donation domain in `packages/worker/src/tests/integration/custom-field-option-merge.test.ts`.
- **Erasure × `merge_history`**: constituent-merge snapshots capture full pre/post-merge rows — custom blob included. The erasure transaction **redacts the erased party's side(s) of every `merge_history` row** (direct identifiers, tags, and the `custom` blob nulled; ids, counters, and timestamps retained), so the Art. 5(2) accountability skeleton survives without an unbounded Art. 17 retention gap. The other party's snapshot is untouched. The erasure audit row records `mergeUndoRowsPurged` + `mergeHistoryRowsRedacted` counts — never values.
- **DSAR (Art. 15)**: the constituent export includes custom values with definition labels and option labels resolved — a donor sees "Segment donateur: Grand donateur", not `opt_a1b2c3d4`. **Caveat**: definitions with `exportable=false` are **omitted** from `export.csv` — a complete Art. 15 response must cover those values separately (temporarily mark the definition exportable and re-run the export, or transcribe from the detail view). The export is the DSAR *vehicle*, not automatically the DSAR *totality*.
