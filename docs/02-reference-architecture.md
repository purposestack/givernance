...
- OIDC provider; issues JWTs consumed by `givernance-api`. Organizations feature enabled (ADR-016 / [issue #114](https://github.com/purposestack/givernance/issues/114)).
- **Storage**: writes to its own `givernance_keycloak` logical database with the dedicated `keycloak` role ([ADR-017](./15-infra-adr.md#adr-017-one-logical-database-per-tool--isolate-keycloak-from-the-application-db)). Schema upgrades (e.g. future Keycloak 27 migrations) only touch this database — the app DB is outside its blast radius.
...
- Multi-tenancy inside the shared realm: each Givernance tenant maps 1:1 to a **Keycloak Organization** (KC 26+; ADR-016 / [issue #114](https://github.com/purposestack/givernance/issues/114)). Per-tenant **Identity Provider federation** (Entra ID, Okta, Google Workspace, …) is bound to the Organization; domain routing uses Keycloak's Home IdP Discovery. See `21-authentication-sso.md` §2.1 for the current onboarding architecture
- **Self-serve + enterprise + invitation tracks** (ADR-016 / `22-tenant-onboarding.md`): tenant rows and Keycloak Organizations/IdPs are provisioned by `POST /v1/public/signup` (SMB self-serve) or `POST /v1/admin/tenants` (enterprise); NPO users are then created Just-In-Time on first SSO login from the JWT claims
...
- **FX rate services** (`@givernance/shared/finance`, ADR-032 / Epic #416) — `ExchangeRateService` (Drizzle-backed org-base-currency lookup + donation conversion, used by the API manual-donation path and the worker Stripe-webhook path) and `FxRateService` (Fixer.io + injected ioredis cache, used by the API dashboard total and the worker FX-cache refresh). Both are DI-only (no `env`/DB/redis singleton inside `shared`); each package keeps a thin adapter that pre-binds its own `db`/`redis`/`env` — the API at `modules/finance/exchange-rate-service.ts` + `lib/fx-rate-service.ts`, the worker at `lib/exchange-rate-service.ts` + `lib/fx-rate-service.ts`.
...
- DB-touching functions take a `db: NodePgDatabase` (the finance/reporting module; `ExchangeRateService` takes a `dbClient`, accepting a per-call transaction for tenant-scoped lookups).
- Redis-touching functions take an injected ioredis-compatible client (`FxRateService` takes a `redis` + `apiKey`).
