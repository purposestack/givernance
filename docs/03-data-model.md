...
| One logical database per tool | All tables described in this document live in the `givernance` database. Keycloak has its own `givernance_keycloak` database with owner role `keycloak` — see [ADR-017](./15-infra-adr.md#adr-017-one-logical-database-per-tool--isolate-keycloak-from-the-application-db). Drizzle schema and migrations are scoped to `givernance` and must never introspect or mutate `givernance_keycloak`. The two DBs follow different role models: `givernance` uses the 3-role RLS pattern (`givernance` owner with `BYPASSRLS`, `givernance_app` runtime without, plus the cluster superuser — see [06 §3.2](./06-security-compliance.md#multi-tenant-security-model)), while `givernance_keycloak` is a single-tenant tool and only has one owner role (`keycloak`) with no RLS — isolation is provided by the DB-level boundary, not by policies. |
