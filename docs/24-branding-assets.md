...
- **SVG sanitisation** (defense in depth). Even though SVG is text and the threat model is XSS-against-the-uploader rather than donor-PII-leakage, the strict allowlist removes `<script>`, `<foreignObject>`, external `<use href>`, and `on*` event handlers before storing the original.
